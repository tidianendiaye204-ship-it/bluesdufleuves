...
export const Route = createFileRoute("/blues-du-fleuve")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Festival fondé par Baaba Maal, célébrant l'intégration et la solidarité des pays de la vallée du fleuve." },
    ],
  }),
...
const artistes = [
  { nom: "Baaba Maal", role: "Fondateur · Légende", origine: "Podor, Sénégal", desc: "Baaba Maal est né 1953 à Podor dans la province du Fouta au Sénégal. Il fait partie du peuple Toucouleur ou Haalpulaar (ceux qui parlent le pulaar), des Peuls du nord du pays." },
  { nom: "Mia Guissé", role: "Auteure-compositrice", origine: "Tambacounda, Sénégal", desc: "Née à Tambacounda, au Sénégal, Aïssata Guissé aka Mia Guissé est auteure-compositrice, interprète et styliste naviguant entre mbalax, musique acoustique, folk-pop, afro-pop, afrobeat." },
  { nom: "JEEBA", role: "Auteur-compositeur", origine: "Thiès, Sénégal", desc: "Jeeba ou Jeeba Abdn de son vrai nom Djibril Ba, né à Thiès, est un auteur-compositeur-interprète et chanteur sénégalais. Il développe une musique qu'il appelle « Jolofbeats »." },
...
const piliers = [
  { Icon: Music2, titre: "Arts Vivants", desc: "Musique et danses traditionnelles représentatives de la richesse Halpular et Toucouleur." },
  { Icon: Palette, titre: "Artisanat", desc: "Exposition du savoir-faire local et des objets d'art traditionnels de la vallée." },
  { Icon: Mic, titre: "Conférences", desc: "Panels de développement et réflexions thématiques sur les préoccupations des populations." },
];
...
const galleryImages = [
  ...
  { id: 4, src: instrumentsImg, alt: "Instruments traditionnels", span: "md:col-span-1 md:row-span-1" },
    ...
    <>
      ...
      <section className="border-b border-border bg-card/30">
        <div className="container-page py-16">
          <p className="text-xs uppercase tracking-[0.3em] text-secondary mb-3">Thème · 16ème Édition</p>
      ...
      <section className="container-page py-20">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {artistes.map((a) => (
            <article key={a.nom} className="rounded-2xl border border-border bg-card overflow-hidden transition hover:border-primary">
      ...
      <section className="container-page py-20 border-t border-border">
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
          {galleryImages.map((img) => (
            <div key={img.id} className={`rounded-2xl overflow-hidden border border-border relative group ${img.span || ""}`}>
              <img 
                src={img.src} 
                alt={img.alt} 
      ...
      <section className="container-page py-20 border-t border-border">
        <div className="rounded-3xl overflow-hidden border border-border aspect-21/9 mb-12">
          <img src={crowdImg} alt="Foule au festival" className="h-full w-full object-cover" loading="lazy" />
        </div>
      ...
      <section className="container-page py-20 border-t border-border">
        <div className="max-w-2xl">
          <h2 className="font-display text-3xl md:text-4xl font-bold">Demande d'accréditation média</h2>
          <p className="mt-2 text-muted-foreground">Journalistes, photographes et équipes presse — envoyez votre demande pour couvrir l'événement ou téléchargez la fiche.</p>
          <div className="mt-6">
            <a 
      ...
      <section className="container-page py-20 border-t border-border">
        <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-10">Nos partenaires</h2>
      ...
      <section className="container-page py-20 border-t border-border bg-card/30">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            Nous sommes à votre disposition. Notre back office se chargera de répondre à vos demandes.
          </p>
          <div className="mt-8 flex flex-col md:flex-row justify-center gap-8 md:gap-16">
            <div className="flex flex-col items-center gap-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-primary">Adresse</span>
              <span className="text-lg font-medium">Dakar, Dakar - Sénégal</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-primary">Téléphone</span>
              <a href="tel:+221774967531" className="text-lg font-medium hover:text-primary transition whitespace-nowrap">+221 77 496 75 31</a>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-primary">Email</span>
              <a href="mailto:contact@bluesdufleuve.sn" className="text-lg font-medium hover:text-primary transition">contact@bluesdufleuve.sn</a>
            </div>
          </div>
          <div className="mt-10 flex justify-center gap-6">
            <a href="https://www.facebook.com/festivalbluesdufleuve" target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-primary transition">Facebook</a>
            <a href="https://www.instagram.com/nannkmedia" target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-primary transition">Instagram</a>
            <a href="https://www.youtube.com/@nannktv" target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-primary transition">Youtube</a>
          </div>
        </div>
      </section>
    </>
