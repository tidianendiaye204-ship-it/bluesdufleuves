import { x as getDefaultExportFromCjs } from "./server-DUqS4k7t.js";
...
class SQLiteDialect {
  ...
  buildFromTable(table2) {
    if (is(table2, Table) && table2[Table.Symbol.IsAlias]) {
      return sql`${sql`${sql.identifier(table2[Table.Symbol.Schema] ?? "")}.`.if(table2[Table.Symbol.Schema])}${sql.identifier(
        table2[Table.Symbol.OriginalName]
      )} ${sql.identifier(table2[Table.Symbol.Name])}`;
    }
    return table2;
  }
  buildSelectQuery({
...
function drizzle$1(client, config = {}) {
  ...
  const session = new SQLiteD1Session(client, dialect, schema2, { logger, cache: config.cache });
...
function requireBindings() {
  ...
  (function(module, exports) {
    var fs = require$$0, path = require$$1, fileURLToPath = requireFileUriToPath(), join = path.join, dirname = path.dirname, exists2 = fs.accessSync && function(path2) {
      try {
        fs.accessSync(path2);
      } catch (e) {
        return false;
...
const inscriptions = sqliteTable("inscriptions", {
...
function getDb() {
  ...
  const d1Binding = process.env.DB || process.env.bluesdufleuve_db;
  if (d1Binding) {
    dbInstance = drizzle$1(d1Binding, { schema });
    return dbInstance;
  } else {
