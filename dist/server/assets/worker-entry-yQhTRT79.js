...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-Dot0zqUM.js").then((n) => n.a2).then(
      (m) => m.default ?? m
    );
  }
  ...
  async fetch(request, env2, ctx) {
    try {
      if (typeof process !== "undefined" && process.env) {
        ...
        globalThis.process = { env: { ...env2 } };
      }
      if (env2?.MY_RATE_LIMITER) {
        const ip = request.headers.get("CF-Connecting-IP") || "unknown";
        const { success } = await env2.MY_RATE_LIMITER.limit({ key: ip });
        if (!success) {
